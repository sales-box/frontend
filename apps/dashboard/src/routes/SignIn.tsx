  ...
  return (
    <AuthLayout onBack={() => onNav("landing")}>
      <div className="mb-8">
        ...
        <p className="text-[15px] text-text-secondary">Sign in to your admin account.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <Btn type="submit" size="lg" loading={submitting} disabled={submitting} className="w-full mt-1">
          Sign in
        </Btn>
      </form>
      ...
      <div className="flex items-center gap-3 my-6">
        ...
        <span className="text-[12px] font-medium text-text-tertiary uppercase tracking-wider">or</span>
        <div className="flex-1 h-px bg-border" />
      </div>
      ...
      <button
        ...
        className={`w-full flex items-center justify-center gap-3 bg-surface border border-border rounded-sm px-4 py-3 text-[14px] font-semibold text-text-primary hover:bg-surface-secondary hover:border-border-focus/40 transition-all duration-200 cursor-pointer shadow-1 hover:shadow-2 ${focusRing}`}
      >
      ...
      <p className="text-center text-[14px] text-text-secondary mt-6">
        ...
        <button onClick={() => onNav("signup")} className={`text-secondary font-semibold hover:underline cursor-pointer ${focusRing}`}>
